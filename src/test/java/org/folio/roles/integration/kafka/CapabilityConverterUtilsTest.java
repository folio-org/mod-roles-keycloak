...
@UnitTest
class CapabilityConverterUtilsTest {

  @ParameterizedTest(name = "[{index}] permission = {0}")
  @CsvSource(nullValues = {"null"}, value = {
    // matched data capabilities
    ...
    //matched procedural capabilities
    ...
    // checks from mapping-overrides
    "perms.users, User Permissions, DATA, MANAGE"
  })
  @DisplayName("getRawCapability_parameterized")
  void getRawCapability_parameterized(String permissionName,
    String expectedResource, CapabilityType expectedType, CapabilityAction expectedAction) {
    var rawCapability = CapabilityConverterUtils.getRawCapability(permissionName);
    assertThat(rawCapability.getResource()).isEqualTo(expectedResource);
    assertThat(rawCapability.getType()).isEqualTo(expectedType);
    assertThat(rawCapability.getAction()).isEqualTo(expectedAction);
  }
  ...
  @DisplayName("getRawCapability_parameterized")
  void hasRequiredFields_parameterized(String res, CapabilityType type, CapabilityAction action, boolean expected) {
    var capability = new Capability().resource(res).type(type).action(action);
    var result = CapabilityConverterUtils.hasRequiredFields(capability);
