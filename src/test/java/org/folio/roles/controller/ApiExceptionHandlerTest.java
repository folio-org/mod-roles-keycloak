...
class ApiExceptionHandlerTest {
  ...
  @Test
  void handleServiceException_positive() throws Exception {
    when(testService.getTestValue()).thenThrow(new ServiceException("service error", "cause", "error"));
    mockMvc.perform(get("/tests").queryParam("query", "cql.allRecords=1").contentType(APPLICATION_JSON))
      .andExpect(status().isBadRequest())
      .andExpect(jsonPath("$.total_records", is(1)))
      ...
      .andExpect(jsonPath("$.errors[0].parameters[0].value", is("error")));
  }
  ...
  @Test
  void handleEntityFeignNotFoundException_positive() throws Exception {
    when(testService.getTestValue()).thenThrow(
      new KeycloakApiException("Role not found", new RuntimeException("Bad request"), 400));
    mockMvc.perform(get("/tests").queryParam("query", "cql.allRecords=1").contentType(APPLICATION_JSON))
      .andExpect(jsonPath("$.total_records", is(1)))
