...
| LOADABLE_ROLE_PERMISSIONS_RETRY_MAX_ATTEMPTS     | 5                                                                                                                                                      |  false   | Max retries to assign capabilities and capabilities set for loadable role                                                               |
| LOADABLE_ROLE_PERMISSIONS_RETRY_BACKOFF_DELAY_MS | 240000                                                                                                                                                 |  false   | Delay between retries to assign capabilities and capabilities set for loadable role                                                     |
...
| KC_CONCURRENCY_THREAD_POOL_SIZE   | Maximum number of concurrent virtual threads for parallel Keycloak operations. Backed by a fixed-size pool of virtual threads (`Thread.ofVirtual()`); tasks queue when all slots are busy so Keycloak is never overwhelmed. Default: `20` |
